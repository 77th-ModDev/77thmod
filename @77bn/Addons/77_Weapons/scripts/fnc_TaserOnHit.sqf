...
if( (_ammo == "NarqDart") || {_weapon == "OPTRE_M6C_Narq"} )then
{
	...
	[_victim,_shooter,_oldTasedUntilTime] spawn 
	{
		...
		{
			[_victim,"Acts_Stunned_Unconscious"] remoteExecCall ["switchMove",0,false];
			sleep 7.032;
			[_victim,"Acts_Stunned_Unconscious_end"] remoteExecCall ["switchMove",0,false];
			...
			waitUntil { (CBA_missionTime > _time + 0.99) || { animationState _victim == "Acts_Stunned_Unconscious_end" } };
			private _weap = currentWeapon _victim;
